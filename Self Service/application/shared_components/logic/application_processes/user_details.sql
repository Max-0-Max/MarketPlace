...
wwv_flow_imp_shared.create_flow_process(
 p_id=>wwv_flow_imp.id(15800712574490674)
...
'  ',
'  CURSOR c_dtl IS',
'  SELECT initcap(x.full_name), x.person_type, x.staff_category, x.department,y.staff_id, x.dept_Code, x.organization_id,x.organization_id,nvl(y.SUB_ORG,''0'')SUB_ORG, nvl(y.DEPARTMENT,''0'')DEPARTMENT, nvl(y.unit,''0'')unit, x.company_name ',
'   FROM xxhrms.xxhrms_master_all x, xxadm.xxadm_password y',
'  WHERE y.staff_id = x.employee_number(+)',
'     AND CASE',
'      when UPPER(:APP_USER) != ''GLOBAL_USER'' and x.organization_id in (Select  column_value FROM TABLE(apex_string.split(y.COMPANY_CODE,'':''))  where rownum=1) THEN 1',
'      WHEN UPPER(:APP_USER) = ''GLOBAL_USER'' THEN 1',
'      ELSE 0',
'      END = 1',
'      ',
...
'        EMPLOYEETYPE_LABEL,',
'        COMPANYNAME_LABEL,',
'        EMPLOYEE_COMMENT_LABEL,',
'        SUPERVISOR_COMMENT_LABEL',
...
'  --',
'  OPEN c_dtl;',
'  FETCH c_dtl INTO :AP_FULL_NAME, :AP_TYPE, :AP_CATEGORY, :AP_DEPARTMENT, :AP_STAFF_ID, :AP_DEP_CODE, :AP_COMPANY_CODE,:SS_COMPANY_CODE,:AP_USUB_ORG,:AP_UDEPT,:AP_UUNIT, :AP_COMPANY_NAME;',
'  CLOSE c_dtl; ',
'  --',
'  OPEN c_instance;',
...
'  :AP_EmployeeType,',
'  :AP_ComapanyName,',
'  :AP_EMPLOYEE_COMMENT_LABEL,',
'  :AP_SUPERVISOR_COMMENT_LABEL;',
'  ',
'CLOSE c_lbl; ',
...
'  :AP_EMPLOYEE_COMMENT_LABEL := NVL(:AP_EMPLOYEE_COMMENT_LABEL,''Employee Comment'');',
'  :AP_SUPERVISOR_COMMENT_LABEL := NVL(:AP_SUPERVISOR_COMMENT_LABEL, ''Supervisor Comment'');',
'  :AP_TOKEN_STATUS := ''INVALID'';',
'  ',
'  --                ',
'  ',
'  --',
