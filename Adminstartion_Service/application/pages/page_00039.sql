...
wwv_flow_imp_page.create_page(
 p_id=>39
,p_name=>'Marketplace Company Setup'
,p_alias=>'YU33'
,p_step_title=>'Marketplace Company Setup'
...
unistr('      <td><a href="#" style="text-decoration:none; color: #7A7A7A; font:Helvetica; font-weight:700; font-size: 14px;" onclick="return redirectToModalPage40UpdateProduct(40, ${item.companyID}, ${item.productID})">Edit \270E</a></td>'),
'    `;',
...
'      <td><span class="status ${item.status.toLowerCase()}">${item.status}</span></td>',
unistr('        <td><a href="#" style="text-decoration:none; color: #7A7A7A; font:Helvetica; font-weight:700; font-size: 14px;" onclick="return redirectToModalPage47UpdateProduct(47, ${item.companyID}, ${item.userId})">Edit \270E</a></td>'),
'    `;',
...
'const redirectToModalPage40UpdateProduct = (pageID, compID, productID) => {',
'        var url = `f?p=#APP_ID#:${pageID}:#SESSION#::NO:RP,${pageID}:P40_COMPANY_ID,P40_PRODUCT_ID:${compID},${productID}`; //''f?p='' || l_app || '':''||apex_application.g_x01||'':''||l_session||''::NO::''||l_item_name||'':''||apex_application.g_x02,',
...
'};',
'',
'const redirectToModalPage47CreateUser = (pageID, compID, compType) => {',
'        var url = `f?p=#APP_ID#:${pageID}:#SESSION#::NO:RP,${pageID}:P47_COMPANY_ID,P47_COMPANY_TYPE:${compID},${compType}`; //''f?p='' || l_app || '':''||apex_application.g_x01||'':''||l_session||''::NO::''||l_item_name||'':''||apex_application.g_x02,',
'        //var url = "f?p=#APP_ID#:"+''201''+":#SESSION#::NO:RP,201:";',
'        url = url.replace("#APP_ID#",&APP_ID.);',
'',
'        url = url.replace("#SESSION#", &SESSION.);',
...
'};',
'',
'const redirectToModalPage47UpdateProduct = (pageID, compID, userID) => {',
'        var url = `f?p=#APP_ID#:${pageID}:#SESSION#::NO:RP,${pageID}:P47_COMPANY_ID,P47_USER_ID:${compID},${userID}`; //''f?p='' || l_app || '':''||apex_application.g_x01||'':''||l_session||''::NO::''||l_item_name||'':''||apex_application.g_x02,',
'        //var url = "f?p=#APP_ID#:"+''201''+":#SESSION#::NO:RP,201:";',
'        url = url.replace("#APP_ID#",&APP_ID.);',
'',
'        url = url.replace("#SESSION#", &SESSION.);',
...
'};',
'// URL Builders',
'function generateNewModalURL(pageID, compID) {',
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44005407895012819)
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44005578773012820)
,p_plug_name=>'New'
,p_region_template_options=>'#DEFAULT#'
...
'             AND COMPANY_ID = :P39_COMPANY_ID',
'             ORDER BY 1',
') LOOP',
'  RETURN',
'        ''<div class="top-region-header">',
'                <div>    ',
'                    <button class="icon-button" onclick="return redirectToMainPageFunc(26)">',
...
'                    <div class="img-div">',
'                        <img class ="prod-img" src="https://testapp.softsuite.com/softsuite/api/mrkt/getimages/''||:P39_COMPANY_ID||''" alt="group">',
'                    </div>',
'                    <div class="text-set">',
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44006493551012829)
,p_plug_name=>'New'
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44007397123012838)
,p_plug_name=>'New'
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44007472563012839)
,p_plug_name=>'Description'
,p_parent_plug_id=>wwv_flow_imp.id(44007397123012838)
,p_region_template_options=>'#DEFAULT#'
...
'              benefits',
'             FROM XXMRKT.XXMRKT_COMPANY_LIST',
'             WHERE COMPANY_ID = :P39_COMPANY_ID',
'              and company_code = :AP_cOMPANY_cODE)',
'    loop',
'    ',
'        RETURN ''<div class = "container">',
'                    <button class="edit-button" onclick="return generateNewModalURL(37,''||i.company_id||'')"> ',
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44007594058012840)
...
'    v_item         VARCHAR2(100) := ''''''P39_PROD_OFFSET'''''';',
'    v_region         VARCHAR2(100) := ''''''products'''''';',
'    v_last_shown    NUMBER := 0;',
'BEGIN',
'',
'    BEGIN',
'        v_offset := TO_NUMBER(:P39_PROD_OFFSET);',
'    EXCEPTION',
...
'    WHERE A.COMPANY_ID = :P39_COMPANY_ID',
...
'				<h2>Products</h2>',
'				<div>',
'				<input type="text" id="product-search" placeholder="Search products..." onkeyup="return filterProducts()" class="search-input">',
'',
'				</div>',
'			  </div>',
'',
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(44007708667012842)
...
wwv_flow_imp_page.create_page_plug(
 p_id=>wwv_flow_imp.id(67009329613789920)
,p_plug_name=>'Users'
,p_parent_plug_id=>wwv_flow_imp.id(44007397123012838)
,p_region_template_options=>'#DEFAULT#:t-Region--removeHeader:t-Region--scrollBody'
,p_plug_template=>wwv_flow_imp.id(5452712612166699)
...
'    v_type         NUMBER; -- 1 is for loan services',
'    v_count        NUMBER;',
'    v_offset       NUMBER := NVL(:P17_USERS_OFFSET, 1);',
'    v_page_size    NUMBER := 5;',
'    v_process      VARCHAR2(200) := ''''''GET_USER_DATA'''''';',
'    v_item         VARCHAR2(100) := ''''''P39_USERS_OFFSET'''''';',
...
'                  <button class="add-btn" onclick="return redirectToModalPage47CreateUser(47,'' || :P39_COMPANY_ID || '', ''||v_type||'')">Add New User</button>',
'                </div>',
'              </div>',
'',
'              <div class="table-wrapper">',
...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(44006506377012830)
,p_name=>'P39_COMPANY_ID'
,p_item_sequence=>10
,p_item_plug_id=>wwv_flow_imp.id(44006493551012829)
,p_display_as=>'NATIVE_HIDDEN'
,p_attributes=>wwv_flow_t_plugin_attributes(wwv_flow_t_varchar2(
  'value_protected', 'Y')).to_clob
);
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(75567080610060739)
,p_name=>'P39_PROD_OFFSET'
,p_item_sequence=>20
,p_item_plug_id=>wwv_flow_imp.id(44006493551012829)
,p_item_default=>'1'
,p_display_as=>'NATIVE_HIDDEN'
...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(75567221386060741)
,p_name=>'P39_USERS_OFFSET'
,p_item_sequence=>30
,p_item_plug_id=>wwv_flow_imp.id(44006493551012829)
,p_item_default=>'1'
,p_display_as=>'NATIVE_HIDDEN'
...
wwv_flow_imp_page.create_page_da_event(
 p_id=>wwv_flow_imp.id(44417073103302669)
,p_name=>'New'
,p_event_sequence=>10
,p_bind_type=>'bind'
...
wwv_flow_imp_page.create_page_da_action(
 p_id=>wwv_flow_imp.id(44417579332302671)
,p_event_id=>wwv_flow_imp.id(44417073103302669)
,p_event_result=>'TRUE'
...
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(48418753412508935)
...
'',
'  :P39_PROD_OFFSET := apex_application.g_x02;',
'',
...
,p_internal_uid=>48418753412508935
);
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(67072305673763550)
...
'    v_page_size number := 5;',
'BEGIN',
'  :P39_USERS_OFFSET := apex_application.g_x02;',
'  -- Open JSON array',
'  apex_json.open_array;',
'',
'  FOR pro_rec IN (',
...
''))
,p_process_clob_language=>'PLSQL'
,p_internal_uid=>67072305673763550
);
